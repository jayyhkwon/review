...
- 멀티 스레드 테스트
  - **애플리케이션 로직**  과 **쓰레드 로직** 을 분리한다.
  - 애플리케이션 로직을 먼저 검증하고 테스트에 통과되면 쓰레드 로직을 테스트 할때 스텁 처리한다.
    - 예제에서는 process() 메서드를 BiConsumer Functional Interface로 stubbing 처리함.

- 데이터 베이스 테스트
  - 클린 룸 데이터베이스 테스트
    - @Before, @After로 테스트 케이스별로 롤백한다.(deleteAll)



- 관심사를 분리하라
  - 애플리케이션 로직은 스레드, 데이터베이스 혹은 문제를 일으킬 수 있는 다른 의존성과 분리해라
